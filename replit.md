...
- **OperatorOS SSO** (`server/auth/sso.ts`): Accepts HS256 JWT handoffs at `GET /sso?token=...` from the OperatorOS parent platform. Verifies signature/issuer (`operatoros`)/audience (`techdeck`)/env/exp/iat (≤90s, ±5s skew), POSTs to `{OPERATOROS_API_URL}/v1/modules/sso/consume` for single-use enforcement, lazily provisions user (keyed on email) + tenant (keyed on `organization_id` slug) + membership (role mapped from claim), then mints a fresh express-session via `regenerate`+`save` and redirects to `/`. Coexists with email/password+TOTP login. SSO is optional: if `MODULE_SSO_SECRET` is unset the route returns `503 sso_not_configured`; if set the secret must be ≥16 chars and `OPERATOROS_BASE_URL`, `OPERATOROS_API_URL`, `OPERATOROS_SSO_ENV` must all be present (audience defaults to `techdeck`). Reject codes: `missing_token`/`bad_request` (400), `signature_invalid`/`issuer_mismatch`/`audience_mismatch`/`env_mismatch`/`expired`/`clock_skew`/`consume_failed` (401), `sso_consume_unavailable` (502). Upstream consume errors mapped: `TOKEN_UNKNOWN`/`TOKEN_REPLAYED`→`consume_failed`, `TOKEN_EXPIRED`→`expired`, `AUDIENCE_MISMATCH`/`ENV_MISMATCH`→matching codes, 5xx→`sso_consume_unavailable`. Token never persisted; `MODULE_SSO_SECRET` and `req.query.token` redacted in logs. Module slug is lowercase `techdeck`. Tests at `tests/sso.test.ts` (14 cases covering verify + consume).
