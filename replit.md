...
- **Billing Module (Task #12 — OperatorOS-managed)**: OperatorOS is the authority for plans, subscriptions, and entitlements. Tech Deck no longer owns plan switching, checkout, or the customer portal:
    ...
    - The Stripe webhook still records events for audit but no longer pauses/unpauses tenants — `pausedAt` is never written. `tenant_subscriptions.status` is updated for legacy reads only.
...
- **Legacy Billing Module (pre-Task-#12)**: The original Stripe-driven subscription system is deprecated but still installed. Stripe webhook continues to record events; tenant_subscriptions table remains for audit and back-compat.
