...
async function validClientAndSite(tenantId: string, clientId?: string | null, siteId?: string | null) {
  ...
  if (siteId) {
    ...
    if (clientId && site.clientId && site.clientId !== clientId) return "Site does not belong to the selected client";
  }
  return null;
}
...
export function registerOperationsRoutes(app: Express) {
  app.get("/api/ops/summary", ...staff, async (req: any, res) => {
    ...
    const [itemsCount, docsCount, contactsCount, dueItems, recentDocs, byClient] = await Promise.all([
      ...
      db.select({ count: sql<number>`count(*)::int` }).from(documentationPages).where(eq(documentationPages.tenantId, tenantId)),
      ...
      db.select().from(documentationPages).where(eq(documentationPages.tenantId, tenantId))
      ...
      db.select({ clientId: configurationItems.clientId, clientName: clients.name, count: sql<number>`count(*)::int` })
        .from(configurationItems).leftJoin(clients, eq(configurationItems.clientId, clients.id))
    ...
    res.json({
      ...
      dueItems,
      recentDocuments: recentDocs.filter((doc) => canViewDocument(req.tenantCtx.role, doc.minimumRole)),
  ...
  app.get("/api/ops/contacts", ...staff, async (req: any, res) => {
    ...
    const rows = await db.select({ contact: contactRecords, clientName: clients.name, siteName: sites.name })
      .from(contactRecords).leftJoin(clients, eq(contactRecords.clientId, clients.id))
      .leftJoin(sites, eq(contactRecords.siteId, sites.id)).where(and(...conditions))
  ...
  app.patch("/api/ops/contacts/:id", ...staff, requireNotPaused(), async (req: any, res) => {
    ...
    const invalid = await validClientAndSite(tenantId, parsed.data.clientId, parsed.data.siteId);
    if (invalid) return res.status(400).json({ message: invalid });
    const [record] = await db.update(contactRecords).set({ ...parsed.data, updatedAt: new Date() })
  ...
  app.get("/api/ops/items", ...staff, async (req: any, res) => {
    ...
    const rows = await db.select({ item: configurationItems, clientName: clients.name, siteName: sites.name })
      .from(configurationItems).leftJoin(clients, eq(configurationItems.clientId, clients.id))
      .leftJoin(sites, eq(configurationItems.siteId, sites.id)).where(and(...conditions))
  ...
  app.get("/api/ops/items/:id", ...staff, async (req: any, res) => {
    ...
    const [row] = await db.select({ item: configurationItems, clientName: clients.name, siteName: sites.name })
      .from(configurationItems).leftJoin(clients, eq(configurationItems.clientId, clients.id))
      .leftJoin(sites, eq(configurationItems.siteId, sites.id))
    ...
    const [relationships, attachments] = await Promise.all([
      ...
      db.select({ attachment: operationalAttachments, evidenceTitle: evidenceItems.title, fileName: evidenceItems.fileName })
        .from(operationalAttachments).innerJoin(evidenceItems, eq(operationalAttachments.evidenceItemId, evidenceItems.id))
  ...
  app.post("/api/ops/folders", ...staff, requireNotPaused(), async (req: any, res) => {
    ...
    if (parsed.data.parentId) {
      const parent = await db.select({ id: documentationFolders.id }).from(documentationFolders).where(and(
  ...
  app.get("/api/ops/documents", ...staff, async (req: any, res) => {
    ...
    const rows = await db.select({ page: documentationPages, clientName: clients.name, folderName: documentationFolders.name })
      .from(documentationPages).leftJoin(clients, eq(documentationPages.clientId, clients.id))
      .leftJoin(documentationFolders, eq(documentationPages.folderId, documentationFolders.id))
  ...
  app.get("/api/ops/documents/:id", ...staff, async (req: any, res) => {
    ...
    const [revisions, backlinks, attachments] = await Promise.all([
      ...
      db.select({ id: documentationPages.id, title: documentationPages.title, slug: documentationPages.slug })
  ...
  app.post("/api/ops/attachments", ...staff, requireNotPaused(), async (req: any, res) => {
    ...
    if (parsed.data.documentationPageId) {
      const parent = await db.select({ id: documentationPages.id }).from(documentationPages).where(and(eq(documentationPages.tenantId, tenantId), eq(documentationPages.id, parsed.data.documentationPageId))).limit(1);
      if (!parent[0]) return res.status(404).json({ message: "Document not found" });
    }
  ...
  app.post("/api/ops/import/preview", ...staff, async (req: any, res) => {
    ...
    const existingNames = new Set((await db.select({ name: configurationItems.name }).from(configurationItems)
      .where(eq(configurationItems.tenantId, req.tenantCtx.tenantId))).map((row) => row.name.toLowerCase()));
    const rows = result.data.map((row, index) => {
      ...
      } else if (!(row.client_id ?? "").trim()) errors.push("client_id is required");
    ...
    for (let index = 0; index < parsed.data.rows.length; index += 1) {
      ...
      try {
        if (parsed.data.kind === "contacts") {
          ...
          const invalid = await validClientAndSite(req.tenantCtx.tenantId, data.clientId, data.siteId);
          if (invalid) throw new Error(invalid);
          await db.insert(contactRecords).values({ ...data, tenantId: req.tenantCtx.tenantId });
        } else {
          const data = configurationInput.parse({ name: row.name, itemType: parsed.data.kind === "ip_records" ? (row.item_type || "ip_address") : row.item_type, clientId: row.client_id || null, siteId: row.site_id || null, status: row.status || "active", vendor: row.vendor || null, product: row.product || null, model: row.model || null, serialNumber: row.serial_number || null, ipAddress: row.ip_address || null, macAddress: row.mac_address || null, externalVaultReference: row.external_vault_reference || null, tags: row.tags ? row.tags.split(";").map((tag) => tag.trim()).filter(Boolean) : [], details: {}, notes: row.notes || null });
          const secretError = assertNoSecretMaterial(data);
          if (secretError) throw new Error(secretError);
          const invalid = await validClientAndSite(req.tenantCtx.tenantId, data.clientId, data.siteId);
          if (invalid) throw new Error(invalid);
          await db.insert(configurationItems).values({ ...dates(data), tenantId: req.tenantCtx.tenantId, createdById: req.tenantCtx.userId });
        }
        imported += 1;
      } catch (error: any) {
  ...
  app.get("/api/ops/search", ...staff, async (req: any, res) => {
    ...
    const [items, documents, contactsFound, clientsFound, sitesFound] = await Promise.all([
      ...
      db.select().from(documentationPages).where(and(eq(documentationPages.tenantId, tenantId), or(ilike(documentationPages.title, pattern), ilike(documentationPages.content, pattern)))).limit(20),
    ...
    res.json({ items, documents: documents.filter((doc) => canViewDocument(req.tenantCtx.role, doc.minimumRole)), contacts: contactsFound, clients: clientsFound, sites: sitesFound });
  });
}
